...
export function GradientShineButton({ children, className }: GradientShineButtonProps) {
    return (
        <button
            type="button"
            className={cn(
                "relative inline-flex items-center justify-center px-8 py-4 overflow-hidden font-medium text-primary-on-accent duration-moderate ease-out rounded-lg group hover:ring-[3px] hover:ring-focus-primary",
                className
            )}
        >
            ...
            <span className="absolute bottom-0 right-0 block w-64 h-64 mb-32 mr-4 duration-slow origin-bottom-left transform rotate-45 translate-x-24 bg-shine rounded-full opacity-30 group-hover:rotate-90 ease" />
